...
let  containsForbiddenWord = (text, forbiddenWord) => text.toLowerCase().includes(forbiddenWord.toLowerCase());

console.log(containsForbiddenWord("Barev luys", "Luys"));
...
let getLastNCharacters = (text, n) => { return n > text.length ?  text : text.slice(-n);}

console.log (getLastNCharacters  ("anapat", 65));
...
let extractMiddle = (text) => { 
    let len = text.length;
    let mid = Math.floor(len / 2);
    
    return  text.length % 2 === 0 ?
    text.substring(mid - 1, mid + 1):
    text.substring(mid, mid + 1);
};


console.log(extractMiddle("ahavorr"));
